...
export default async function FollowUpsPage() {
  ...
  const [{ data: projects }, { data: followUps }] = await Promise.all([
    ...
    supabase.from("follow_ups").select("followup_id,project_id,followup_date,followup_time,followup_type,result,notes,next_action_date,next_action_type,next_action_status,completed_at,completed_result,completed_notes").order("followup_date",{ascending:true}).limit(500)
  ]);
