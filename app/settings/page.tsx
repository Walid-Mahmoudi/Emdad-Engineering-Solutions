  ...
  return (
    <main className="nexus-page settings-workspace">
      ...
      <section className="settings-summary-grid">
        <div className="nexus-card settings-summary"><Database size={18} /><div><span>CRM</span><strong>{settings.get("CRM Name") || "EMDAD NEXUS"}</strong><small>{settings.get("Company") || "EMDAD Engineering Solutions"}</small></div></div>
        ...
        <div className="nexus-card settings-summary"><SlidersHorizontal size={18} /><div><span>Pipeline</span><strong>6 stages</strong><small>Sunday week start</small></div></div>
      </section>

      <section className="nexus-card">
        <div className="section-head"><div><h2>Configuration</h2><p className="muted">Current values preserved from the CRM configuration.</p></div></div>
        <div className="nexus-table-wrap">
          <table className="nexus-table">
            <thead><tr><th>Setting</th><th>Value</th></tr></thead>
            <tbody>
              {rows.map((row) => <tr key={row.type}><td>{row.type}</td><td>{row.value || "—"}</td></tr>)}
            </tbody>
          </table>
        </div>
      </section>
