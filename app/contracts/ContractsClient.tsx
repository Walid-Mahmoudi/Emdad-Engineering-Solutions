...
export default function ContractsClient({contracts,collections}:{contracts:any[];collections:any[]}){
 ...
 async function remove(id:string){if(!confirm("Delete this contract? All linked collections will also be deleted."))return;setBusy(true);try{await deleteContract(id);location.reload()}catch(e:any){alert(e.message);setBusy(false)}}
 return <section className="nexus-card contract-workspace-card">
  ...
  <tbody>{contracts.map(c=>{const value=Number(c.contract_value||0);const collected=Math.min(collectedByContract.get(c.contract_id)||0,value);const remaining=Math.max(value-collected,0);const pct=value?Math.round(collected/value*100):0;return <tr key={c.contract_id}>
   ...
   <td><button className="icon-button" disabled={busy} title="Delete contract" onClick={()=>remove(c.contract_id)}><Trash2 size={15}/></button></td>
