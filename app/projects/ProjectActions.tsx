...
export default function ProjectActions({project, followUps, contract, collected}:{project:{project_id:string;current_action:string|null;estimated_value:number|null};followUps:Array<{followup_id:string;followup_type:string|null;result:string|null;followup_date:string;completed_at:string|null}>;contract:{contract_id:string;contract_value:number}|null;collected:number}) {
  ...
  async function saveCollection(){
    ...
    await withBusy(async()=>{
      try{
        ...
        await addCollection({projectId:project.project_id,contractId:contract.contract_id,date:collectionDate,amount:Number(collectionAmount),paymentMethod,status:collectionStatus,dueDate:collectionDueDate,quarter:collectionQuarter,notes:collectionNotes});
  ...
  return <section className="card">
    ...
    {project.current_action==="Closed Won"&&<><hr style={{margin:"20px 0",border:0,borderTop:"1px solid #e5e7eb"}}/><h3>Contract</h3>
      ...
      {contract&&<><h3 style={{marginTop:18}}>Add Collection</h3><div className="nexus-form-grid"><label>Date<input type="date" value={collectionDate} onChange={e=>setCollectionDate(e.target.value)}/></label><label>Amount<input type="number" min="0" value={collectionAmount} onChange={e=>setCollectionAmount(e.target.value)}/></label><label>Payment Method<input value={paymentMethod} onChange={e=>setPaymentMethod(e.target.value)}/></label><label>Status<select value={collectionStatus} onChange={e=>setCollectionStatus(e.target.value)}>{COLLECTION_STATUSES.map(s=><option key={s}>{s}</option>)}</select></label><label>Due Date<input type="date" value={collectionDueDate} onChange={e=>setCollectionDueDate(e.target.value)}/></label><label>Quarter<select value={collectionQuarter} onChange={e=>setCollectionQuarter(e.target.value)}><option value="">Not specified</option>{QUARTERS.map(q=><option key={q}>{q}</option>)}</select></label><label>Notes<input value={collectionNotes} onChange={e=>setCollectionNotes(e.target.value)}/></label><div style={{display:"flex",alignItems:"end"}}><button className="nexus-primary" disabled={busy} onClick={saveCollection}>Record Collection</button></div></div><p className="muted">Collected: {collected.toLocaleString("en-EG")} EGP · Remaining: {Math.max(0,Number(contract.contract_value||0)-collected).toLocaleString("en-EG")} EGP</p>{collectionMsg&&<p className="muted">{collectionMsg}</p>}</>}
    </>}
  </section>;
}
